...
.. code:: bash
   ...
   git clone https://github.com/DCMLab/pleyel_quartets.git
...
* `Ignaz Pleyel – String Quartets <https://github.com/DCMLab/pleyel_quartets/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by four files with identical names, each in its own folder. For example, the first movement of the first quartet, Op. 2, No. 1, B.307 has the following files:

-  ``MS3/b307op2n1a.mscx``: Uncompressed MuseScore file including the music and annotation labels.
-  ``notes/b307op2n1a.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/b307op2n1a.tsv``: A table with relevant information about the measures in the score.
-  ``harmonies/b307op2n1a.tsv``: A list of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/b307op2n1a.tsv')
   notes = ms3.load_tsv('notes/b307op2n1a.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/pleyel_quartets/issues>`__ and feel free to fork and submit pull requests.
